...
def routing_node(state: LeaderAgentState) -> Dict[str, Any]:
    ...
    try:
        ...
        save_node_log(
            ...
            node_name="router",
            step="decide",
            input_data={"title": ctx["title"], "script_content": ctx["script_content"]},
            output_data={"worker_type": worker_type},
            llm_reasoning=response.content
        )
    except Exception as e:
        logger.error(f"Routing Node Error: {e}")
        worker_type = "slideshow" # Safe fallback
        save_node_log(
            job_id=state["job_id"],
            node_name="router",
            step="decide",
            input_data={"title": ctx["title"]},
            output_data={"worker_type": worker_type, "error": str(e)},
            llm_reasoning="LLM failed to provide valid worker_type, using fallback: slideshow",
            log_level="WARNING"
        )
