...
export function AssetTable({ assets, loading, deletingId, onDelete, onUploadClick, currentPath, setCurrentPath, onSelectAsset, multiSelect = false, selectedAssets = [], onToggleAsset }: AssetTableProps) {
  ...
  const allItems = React.useMemo<Array<{ isFolder: boolean; name?: string; asset?: Asset; id: string }>>(() => {
    const folderItems = folders.map(f => ({ isFolder: true, name: f, id: `folder-${f}` }));
    const fileItems = files.map(f => ({ isFolder: false, asset: f, id: f.id }));
                        ...
                        <td className="px-6 py-4">
                          <div className="flex items-center gap-3">
                            ...
                            <div className="flex flex-col">
                              <span className="font-medium text-white line-clamp-1 max-w-[300px]" title={normalizedPath ? (asset.full_path || asset.file_name).slice(normalizedPath.length) : (asset.file_name)}>
                                {normalizedPath ? (asset.full_path || asset.file_name).slice(normalizedPath.length) : (asset.file_name)}
                              </span>
