...
export default function ProjectDetails() {
  ...
  const successJobs = displayJobs.filter(j => j.status === "SUCCESS").length;
  const processingJobs = displayJobs.filter(j => j.status === "PROCESSING").length;
  const failedJobs = displayJobs.filter(j => j.status === "FAILED").length;
      ...
      {displayJobs.length > 0 && (
        <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
          <Card className="p-5 bg-white/5 border-white/10 backdrop-blur-sm flex flex-col justify-center relative overflow-hidden group">
            ...
            <p className="text-3xl font-bold mt-1 text-white relative z-10">{displayJobs.length}</p>
          </Card>
      ...
      <div className="space-y-4">
        <div className="flex items-center justify-between">
          ...
          <div className="flex gap-3">
             ...
             <Link to={`/create-promotion?project=${project.id}`}>
               <Button className="glowing-button text-xs py-1.5 h-9">
        ...
        <Card className="overflow-hidden border-white/10 shadow-2xl">
          {jobsLoading && jobs.length === 0 ? (
            ...
            <div className="flex flex-col items-center justify-center p-16 text-center gap-4 m-4">
               <div>
                 <h4 className="text-lg font-bold text-white mb-1">No videos found</h4>
                 <p className="text-sm text-muted-foreground max-w-sm mx-auto">There are no videos in this format.</p>
               </div>
            </div>
          ) : (
            <JobTable
