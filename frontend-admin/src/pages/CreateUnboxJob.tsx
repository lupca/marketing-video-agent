...
export default function CreateUnboxJob() {
  ...
  const convertToCapCutConfig = (
    ...
    clipUrls.forEach((url, idx) => {
      ...
      const eventsInSeg = events.filter(
        e => e.time >= timeStart && e.time < timeEnd
      )

      const textOverlay = eventsInSeg.map(e => e.text).join(" | ")
      const visualEffects = eventsInSeg.some(e => e.effect === "hook")
        ? ["glitch"]
        : []

      timelineScript.push({
        segment: `seg_${key}_${url.split("/").pop()?.split(".")[0] || "clip"}`,
        video_source: key,
  ...
  const handleCapCutSubmit = async () => {
    ...
    try {
      ...
      const capCutConfigData = convertToCapCutConfig(clipUrls, audioUrl, textEvents, targetProjectId)
      ...
      if (cloneJobId) {
        ...
        await api.patch(`/api/jobs/${cloneJobId}`, unboxDraftPayload)
      }
  ...
  const canGoStep2 = clips && clips.length > 0 && audio;

  return (
    <JobCreationLayout jobType="unbox" tmcpContext={tmcpContext}>
